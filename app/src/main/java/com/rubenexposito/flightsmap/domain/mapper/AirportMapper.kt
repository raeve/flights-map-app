...
class AirportMapper {
    ...
    fun convertOperationsScheduleToScheduleList(operationsSchedulesDto: OperationsSchedulesDto): List<Schedule> = operationsSchedulesDto.scheduleResource.scheduleList.map {
        Schedule(it.totalJourney.duration, it.flightList.map { flightDto -> Flight(PlaceFlight(flightDto.departure.airportCode, flightDto.departure.scheduledTimeLocal.dateTime), PlaceFlight(flightDto.arrival.airportCode, flightDto.arrival.scheduledTimeLocal.dateTime), "${flightDto.marketingCarrier.airlineId}-${flightDto.marketingCarrier.flightNumber}") })
    }
}